...
Page({
  ...
  onHide() {
    ...
    // 注意：需要用户授权"使用小程序期间和离开小程序后"的后台定位权限
    
    // 如果使用后台定位API（wx.startLocationUpdate），定位会继续运行
    // 如果使用传统方式（定时器），定时器可能会被暂停（这是正常的，因为传统方式不支持后台）
    // 我们优先使用后台定位API，所以这里不需要额外处理
  },
  ...
  onUnload() {
    // 页面卸载时，停止定位更新
    // 注意：对于后台定位（wx.startLocationUpdate），即使页面卸载，定位也可能继续运行
    // 但为了资源管理，我们仍然在这里停止定位
    // 如果需要在完全退出小程序后继续定位，需要用户授权"使用小程序期间和离开小程序后"权限
    // 并且在 app.json 中配置 requiredBackgroundModes: ["location"]
    this.stopLocationUpload();
  }
});
